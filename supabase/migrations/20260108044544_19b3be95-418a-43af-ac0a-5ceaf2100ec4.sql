-- Drop the OLD version of mutual_exchange_contact (without shared_card_id parameter)
DROP FUNCTION IF EXISTS public.mutual_exchange_contact(
  p_owner_id uuid,
  p_viewer_id uuid,
  p_viewer_email text,
  p_viewer_name text,
  p_viewer_phone text,
  p_viewer_whatsapp text,
  p_viewer_company text,
  p_viewer_designation text,
  p_viewer_linkedin text,
  p_viewer_website text,
  p_viewer_photo_url text,
  p_viewer_about text,
  p_viewer_synka_user_id uuid
);

-- Create single unified function with shared_card_id support
...
BEGIN
  ...
  ELSE
    -- INSERT new contact with card-centric avatar linking
